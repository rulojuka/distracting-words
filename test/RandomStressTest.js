...
const createHashTable = (existing_codes)=>{
  const hashTable = {}
...
const createSubsequenceCodes = (codeSize, numberOfCodes, possibleDistractingWords) =>{
...
const test = (possibleDistractingWords, codeSize, repeatDictionaryTimes, totalNumberOfExistingRandomCodes, totalNumberOfTestRandomCodes, totalNumberOfSubsequenceCodes) => {
...
const existingRandomCodes = createRandomCodes(codeSize, totalNumberOfExistingRandomCodes)

const testRandomCodes = createRandomCodes(codeSize, totalNumberOfTestRandomCodes)

const subsequenceCodes = createSubsequenceCodes(codeSize, totalNumberOfSubsequenceCodes, possibleDistractingWords)
...
testRandomCodes.forEach(testRandomCode => {
  const returnValue = codeDoesNotExistAndIsNotDistracting(testRandomCode, repeatedDistractingWords, hashTable)
...
subsequenceCodes.forEach(subsequenceCode => {
  const returnValue = codeDoesNotExistAndIsNotDistracting(subsequenceCode, repeatedDistractingWords, hashTable)
